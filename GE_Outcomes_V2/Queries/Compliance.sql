...
SELECT C.course_id, C.num_of_assignments, D.number_of_students, D.outcome_1, D.outcome_2, D.teacher_#1_name, D.teacher_#1_email, C.semester,C.course, C.section, C.college, 'Naughty - Too Many Assessments' as Compliance INTO dbo.compliance
...
SELECT C.course_id, C.num_of_assignments, D.number_of_students, D.outcome_1, D.outcome_2, D.teacher_#1_name, D.teacher_#1_email, C.semester,C.course, C.section, C.college, 'Naughty - Not Enough Assessments' as Compliance 
FROM #Student_Count as C
INNER JOIN registrar_data AS D on C.course_id=D.course_sis_id
...
SELECT C.course_id, C.num_of_assignments, D.number_of_students,D.outcome_1, D.outcome_2, D.teacher_#1_name, D.teacher_#1_email, C.semester,C.course, C.section, C.college, 'Compliant' as Compliance 
FROM #Student_Count as C
INNER JOIN registrar_data AS D on C.course_id=D.course_sis_id
WHERE C.num_of_assignments >=  (0.7) * D.number_of_students
